...
const DashboardPreview = ({ id, t }) => {
  const metrics = [
    { label: t('soilMoisture'), value: "38%", bar: 38 },
    { label: t('temperature'), value: "31°C", bar: 62 },
    { label: t('waterFlow') || "Water Flow", value: "2.4 L/min", bar: 48 },
    { label: t('humidity') || "Humidity", value: "65%", bar: 65 },
  ];
            ...
            <div className="p-6">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div className="rounded-lg border border-border p-4">
                  <p className="text-xs font-semibold text-muted-foreground mb-3">{t('cropHealth') || "Crop Health Map (NDVI)"}</p>
                  ...
                  <div className="flex items-center gap-3 mt-3">
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-[9px] text-muted-foreground">{t('stressed')}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <span className="w-3 h-3 rounded-sm bg-yellow-700" />
                      <span className="text-[9px] text-muted-foreground">{t('moderate')}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <span className="w-3 h-3 rounded-sm bg-green-700" />
                      <span className="text-[9px] text-muted-foreground">{t('healthy')}</span>
                    </div>
                  </div>
                </div>
                <div className="rounded-lg border border-border p-4">
                  <p className="text-xs font-semibold text-muted-foreground mb-3">{t('nextRecommended') || "Irrigation Recommendations"}</p>
                  <div className="space-y-3">
                    {[
                      { zone: `${t('zone')} A`, status: t('irrigate_now'), urgent: true },
                      { zone: `${t('zone')} B`, status: t('adequate'), urgent: false },
                      { zone: `${t('zone')} C`, status: t('schedule_tomorrow'), urgent: false },
                      { zone: `${t('zone')} D`, status: t('irrigate_now'), urgent: true },
                    ].map((z) => (
